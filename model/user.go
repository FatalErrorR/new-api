...
type User struct {
	...
	AffRebateRate    *float64 `json:"aff_rebate_rate" gorm:"column:aff_rebate_rate"`                  // 专属充值返利比例（百分比 0-100，NULL 表示沿用全局）
	InviterId        int      `json:"inviter_id" gorm:"type:int;column:inviter_id;index"`
	// ApplyAffRebateRate is set by the admin update handler when the JSON body
	// included aff_rebate_rate (including explicit null to clear the override).
	ApplyAffRebateRate bool                       `json:"-" gorm:"-"`
...
func (user *User) EditWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	if newUser.ApplyAffRebateRate {
		if err := ValidateAffRebateRate(newUser.AffRebateRate); err != nil {
			return err
		}
		updates["aff_rebate_rate"] = newUser.AffRebateRate
	}
