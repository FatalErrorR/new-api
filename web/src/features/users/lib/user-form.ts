...
export const userFormSchema = z.object({
  ...
  aff_rebate_rate: z.number().min(0).max(100).nullable().optional(),
  admin_permissions: z
...
export function transformFormDataToPayload(
  ...
  if (userId === undefined) {
    ...
    payload.aff_rebate_rate =
      data.aff_rebate_rate === undefined ? null : data.aff_rebate_rate
    payload.id = userId
  }
...
export function transformUserToFormDefaults(user: User): UserFormValues {
  return {
    ...
    remark: user.remark || '',
    aff_rebate_rate: user.aff_rebate_rate ?? null,
